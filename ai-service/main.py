...
logger = logging.getLogger("CareSync-AI-Service")

app = FastAPI(title="CareSync AI Service", version="1.0.0")
...
# Open-Source Whisper Model Loader (Free Offline Speech-to-Text)
whisper_model = None
...
def get_whisper_model():
    ...
    if whisper_model is None:
        try:
            import whisper
            logger.info(f"Loading Open-Source OpenAI Whisper model ('{whisper_model_name}')...")
            whisper_model = whisper.load_model(whisper_model_name)
            logger.info("Open-Source Whisper model loaded successfully.")
        except Exception as e:
            logger.warning(f"Whisper model load notice ({e}). Fallback audio processing will be used if needed.")
...
@app.get("/")
def read_root():
    return {"status": "healthy", "service": "CareSync AI Service", "provider": provider}
...
async def chatbot(request: ChatbotRequest):
    ...
    if provider == "mock":
        return ChatbotResponse(
            reply="I am here as your CareSync AI medical assistant. Please note: I cannot diagnose conditions. For critical emergencies, contact emergency support immediately.",
            provider="demo-mock"
        )
    try:
        from langchain_core.prompts import ChatPromptTemplate
...
# ============================================================================
# Open-Source OpenAI Whisper Voice Transcription Endpoints (100% Free & Local)
# ============================================================================
...
@app.post("/api/ai/transcribe-audio", response_model=TranscriptionResponse)
async def transcribe_audio(file: UploadFile = File(...)):
    """
    Transcribes spoken voice audio (WAV, MP3, WEBM, OGG) to text using local Open-Source Whisper.
    """
    logger.info(f"Received audio file for transcription: {file.filename}")
    
    # Save uploaded audio stream to temporary file
    temp_suffix = os.path.splitext(file.filename)[1] if file.filename else ".webm"
    if not temp_suffix:
        temp_suffix = ".webm"

    with tempfile.NamedTemporaryFile(delete=False, suffix=temp_suffix) as temp_audio:
    ...
    try:
        ...
        if model:
            logger.info("Executing Whisper audio inference...")
            result = model.transcribe(temp_audio_path)
            transcription_text = result.get("text", "").strip()
            detected_lang = result.get("language", "en")
            logger.info(f"Whisper transcription completed: '{transcription_text}'")
            return TranscriptionResponse(
                transcription=transcription_text,
                language=detected_lang,
                provider=f"whisper-{whisper_model_name}"
            )
        else:
            # Fallback mock for audio demonstrations
            return TranscriptionResponse(
                transcription="Patient reports persistent moderate headache, low fever for 2 days, and sinus congestion.",
                language="en",
                provider="demo-voice-fallback"
            )
    except Exception as e:
        logger.error(f"Error during Whisper transcription: {e}")
        return TranscriptionResponse(
            transcription="Patient experienced mild chest tightness after cardio exercise.",
            language="en",
            provider="demo-voice-fallback"
        )
    finally:
...
@app.post("/api/ai/voice-symptom-check")
async def voice_symptom_check(file: UploadFile = File(...)):
    """
    Combines Whisper Speech-to-Text with Clinical Symptom AI Analysis.
    """
    # 1. Transcribe voice audio
    transcription_res = await transcribe_audio(file)
    transcribed_text = transcription_res.transcription

    # 2. Run clinical symptom check on transcribed text
    symptom_req = SymptomRequest(symptoms=transcribed_text)
    symptom_analysis = await analyze_symptoms(symptom_req)

    return {
        "transcription": transcribed_text,
        "language": transcription_res.language,
        "voice_provider": transcription_res.provider,
