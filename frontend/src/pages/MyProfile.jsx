    ...
    return (
        <div className='max-w-2xl mx-auto py-6 space-y-8'>
            <div className="text-left space-y-2">
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white border border-[#d2d2d7]/80 text-[#0071e3] text-xs font-semibold shadow-xs">
                    <Shield className="w-3 h-3" />
                    <span>Patient Profile &amp; Bio-Data</span>
                </div>
                <h1 className="text-3xl font-black tracking-tight text-[#1d1d1f]">Account Settings</h1>
                <p className="text-xs text-[#86868b]">Manage contact credentials, emergency addresses, and clinical attributes.</p>
            </div>

            <div className="bg-white border border-[#e5e5ea] rounded-3xl p-7 sm:p-8 shadow-sm space-y-6">
                
                {/* Photo & Name Section */}
                <div className="flex flex-col sm:flex-row items-center gap-5 pb-6 border-b border-[#e5e5ea]">
                    <div className="relative">
                        <img 
                            className='w-24 h-24 rounded-2xl object-cover border border-[#e5e5ea] bg-[#f5f5f7]' 
                            src={image ? URL.createObjectURL(image) : userData.image || '/fallback-user.png'} 
                            alt="avatar" 
                        />
                        {isEdit && (
                            <label htmlFor='image' className="absolute -bottom-1 -right-1 bg-[#0071e3] text-white p-2 rounded-xl border border-white cursor-pointer shadow-md hover:scale-105 active:scale-95 transition-all">
                    ...
                    <div className="flex-1 text-center sm:text-left space-y-1.5 w-full">
                        {isEdit ? (
                            <input 
                                className='w-full max-w-sm px-3.5 py-2 border border-[#e5e5ea] bg-[#f5f5f7] text-[#1d1d1f] text-sm font-bold rounded-2xl focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#0071e3]/20 transition-all' 
                                type="text"
                                onChange={(e) => setUserData(prev => ({ ...prev, name: e.target.value }))}
                                value={userData.name}
                            />
                        ) : (
                            <h2 className='text-xl font-black text-[#1d1d1f]'>{userData.name}</h2>
                        )}
                        <p className="text-xs text-[#86868b] font-medium">Verified Patient Account</p>
                    </div>
                </div>
                ...
                <div className="space-y-4 text-left">
                    <h3 className="text-xs font-bold uppercase tracking-wider text-[#86868b] flex items-center gap-1.5">
                        <Shield className="w-3.5 h-3.5 text-[#0071e3]" /> Contact Information
                    </h3>
                    
                    <div className="grid grid-cols-1 sm:grid-cols-[150px_1fr] gap-x-6 gap-y-4 text-xs">
                        <div className="font-bold text-[#86868b] flex items-center gap-1">
                            Email address
                        </div>
                        <div className="text-[#1d1d1f] font-medium bg-[#f5f5f7] border border-[#e5e5ea] px-3.5 py-2 rounded-xl max-w-sm truncate select-all">
                            {userData.email}
                        </div>
