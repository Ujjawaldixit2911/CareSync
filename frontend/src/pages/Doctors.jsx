...
const Doctors = () => {
  ...
  const doctors = (doctorsData && doctorsData.length > 0) ? doctorsData : staticDoctors
  ...
  const DoctorCardSkeleton = () => (
    <div className='border border-[#e5e5ea] dark:border-zinc-800 bg-white dark:bg-zinc-900 rounded-2xl overflow-hidden p-3 space-y-3 animate-pulse'>
      <div className='bg-[#eceef1] dark:bg-zinc-800 w-full h-44 rounded-xl'></div>
      <div className='space-y-2 px-1'>
        <div className='bg-[#eceef1] dark:bg-zinc-800 h-3.5 w-1/3 rounded'></div>
        <div className='bg-[#eceef1] dark:bg-zinc-800 h-4.5 w-3/4 rounded'></div>
        <div className='bg-[#eceef1] dark:bg-zinc-800 h-3.5 w-1/2 rounded'></div>
      </div>
    </div>
  )
  ...
  return (
    <div className="space-y-6">
      {/* Title Header */}
      <div>
        <h1 className="text-2xl font-bold tracking-tight text-[#1d1d1f] dark:text-zinc-50">Healthcare Professionals</h1>
      ...
      <div className="flex flex-col sm:flex-row gap-3 justify-between items-stretch sm:items-center">
        ...
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 border border-[#e5e5ea] dark:border-zinc-800 bg-white dark:bg-zinc-900 rounded-xl text-xs focus:outline-none focus:ring-2 focus:ring-blue-500/20 text-zinc-800 dark:text-zinc-100 shadow-xs"
          />
        </div>

        {/* Current Specialty Indicator */}
        {speciality && (
          <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-blue-50 border border-blue-200 text-[#0071e3] text-xs font-semibold rounded-lg self-start">
      ...
      <div className='flex flex-col md:flex-row items-start gap-6'>
        {/* Sidebar Filters */}
        <aside className='w-full md:w-56 flex flex-row md:flex-col gap-2 overflow-x-auto pb-3 md:pb-0 no-scrollbar flex-shrink-0'>
          <button
            onClick={() => navigate('/doctors')}
            className={`whitespace-nowrap px-4 py-2.5 text-left text-xs font-semibold rounded-xl border transition-all cursor-pointer ${
              !speciality
                ? 'bg-[#0071e3] border-[#0071e3] text-white shadow-sm shadow-blue-500/25'
                : 'bg-white dark:bg-[#1f1f23] text-zinc-700 dark:text-zinc-300 border-[#e5e5ea] dark:border-zinc-800 hover:bg-[#f5f5f7] dark:hover:bg-[#27272a]'
            }`}
          >
            All Specialties
          </button>
