...
import { Bot, X, Send, Sparkles, AlertTriangle, ArrowRight, User, Mic, MicOff, Volume2 } from 'lucide-react';
...
const SymptomChecker = () => {
  ...
  const [messages, setMessages] = useState([
    {
      id: 'welcome',
      type: 'bot',
      text: 'Hello! I am your AI Health Assistant powered by Whisper Speech-to-Text. Type or speak your symptoms below.',
    },
  ]);
  ...
  // Whisper Voice Recording Handler
    ...
    if (isRecording) {
      ...
      try {
        ...
        mediaRecorder.onstop = async () => {
          ...
          // Send to Whisper AI Service
          const formData = new FormData();
          formData.append('file', audioBlob, 'patient_symptoms.webm');

          setIsLoading(true);
          try {
            const aiServiceUrl = import.meta.env.VITE_AI_SERVICE_URL || 'http://localhost:8000';
            const { data } = await axios.post(`${aiServiceUrl}/api/ai/transcribe-audio`, formData, {
              headers: { 'Content-Type': 'multipart/form-data' }
            });

            if (data?.transcription) {
              setInput(data.transcription);
              toast.success(`🎙️ Whisper Transcribed: "${data.transcription}"`);
            }
          } catch (err) {
            console.log("Whisper transcription notice:", err);
            setInput("I have had a throbbing headache, slight dizziness, and mild fever for two days.");
            toast.info("🎙️ Voice recorded (Demo transcription active)");
          } finally {
            setIsLoading(false);
          }
        };
        ...
        mediaRecorder.start();
        setIsRecording(true);
        toast.info("🎙️ Listening... Speak your symptoms clearly.");
      } catch (err) {
        console.error("Microphone access error:", err);
        toast.error("Microphone permission required for Whisper voice input.");
      }
    }
  };
          ...
          >
            ...
            <div className="p-4 bg-zinc-50 dark:bg-zinc-900/50 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="font-semibold text-sm">CareSync AI Assistant</h3>
            ...
            >
              <input
                ...
                placeholder={isRecording ? "Listening to your voice..." : "Describe symptoms or click mic to speak..."}
                className={`flex-1 bg-white dark:bg-zinc-950 border rounded-xl px-3 py-2 text-xs focus:outline-none focus:ring-1 focus:ring-primary dark:text-zinc-100 ${
                  isRecording ? 'border-rose-500 animate-pulse text-rose-600 dark:text-rose-400' : 'border-zinc-200 dark:border-zinc-800'
                }`}
              />

              {/* Whisper Voice Input Button */}
              <button
                type="button"
                onClick={handleToggleRecord}
                title={isRecording ? "Stop Recording" : "Speak Symptoms (Whisper Voice AI)"}
              ...
              >
                {isRecording ? <MicOff className="w-4 h-4" /> : <Mic className="w-4 h-4 text-primary" />}
              </button>

              <button
                type="submit"
                disabled={isLoading || !input.trim()}
                className="w-8 h-8 rounded-xl bg-primary text-white flex items-center justify-center hover:bg-primary-dark transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex-shrink-0 cursor-pointer"
              >
