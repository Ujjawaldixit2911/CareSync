...
const RelatedDoctors = ({ speciality, docId }) => {
  ...
  useEffect(() => {
    ...
    if (list.length > 0 && speciality) {
      const doctorsData = list.filter(
        (doc) => doc.speciality.toLowerCase() === speciality.toLowerCase() && String(doc._id) !== String(docId)
      )
      setRelDoc(doctorsData.length > 0 ? doctorsData : list.slice(0, 5))
    }
